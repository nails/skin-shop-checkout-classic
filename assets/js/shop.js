...
_nails_skin_shop_checkout_classic = function() {
    ...
    this.__construct = function() {
        ...
        if ($('.nails-skin-shop-checkout-classic.basket').length) {
            this._basket_init();
        }

        // --------------------------------------------------------------------------

        if ($('.nails-skin-shop-checkout-classic.checkout').length) {
            this._checkout_init();
        }

        // --------------------------------------------------------------------------

        if ($('.nails-skin-shop-checkout-classic.processing').length) {
    ...
    this._checkout_validate_step_1 = function() {
        var _valid  = true;
        var _value  = '';
        var shippingRequired = $('#checkout-step-1').data('shipping-required');
    ...
    this._checkout_validate_step_2 = function() {
        ...
        if (!shippingRequired || $('#same-billing-address').prop('checked') === false) {
            //  Address Line 1
